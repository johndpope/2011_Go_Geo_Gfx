...
func FileExistsPath (dirPath string, fileBaseName string, fileExts []string, tryLower bool, tryUpper bool) (string, time.Time, int64) {
	...
	for _, fext = range fileExts {
		fpath = path.Join(dirPath, fileBaseName + fext)
		if stat, err = os.Stat(fpath); err != nil {
			if tryUpper {
				fpath = path.Join(dirPath, strings.ToUpper(fileBaseName) + fext)
				stat, err = os.Stat(fpath)
			}
			if (err != nil) && tryLower {
				fpath = path.Join(dirPath, strings.ToLower(fileBaseName) + fext)
				stat, err = os.Stat(fpath)
			}
		}
